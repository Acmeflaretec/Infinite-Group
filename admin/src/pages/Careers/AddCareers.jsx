...
const AddCareers = () => {
  ...
  const handleSubmit = async () => {
    try {
      if (!details.title) return toast.error("Job title is required");
      if (!details.location) return toast.error("Job location is required");
      if (!details.type) return toast.error("Job type is required");
      if (!details.summary) return toast.error("Job summary is required");

      setDisable(true);

      const formData = {
        ...details,
        dutiesAndResponsibilities: details.dutiesAndResponsibilities
          ? details.dutiesAndResponsibilities.split("\n").filter(Boolean)
          : [],
        workingConditions: details.workingConditions
          ? details.workingConditions.split("\n").filter(Boolean)
          : [],
        jobRequirements: details.jobRequirements
          ? details.jobRequirements.split("\n").filter(Boolean)
          : [],
      };
  ...
  return (
    <PageLayout title="Add Career">
      <Grid container spacing={3} p={5}>
        ...
        <Grid item xs={12}>
          <Input
            placeholder="Duties and Responsibilities (separate each point with a new line)"
            name="dutiesAndResponsibilities"
            value={details.dutiesAndResponsibilities}
            onChange={handleChange}
            multiline
            rows={5}
          />
        ...
        <Grid item xs={12}>
          <Input
            placeholder="Working Conditions (separate each point with a new line)"
            name="workingConditions"
            value={details.workingConditions}
            onChange={handleChange}
            multiline
            rows={5}
          />
        ...
        <Grid item xs={12}>
          <Input
            placeholder="Job Requirements (separate each point with a new line)"
